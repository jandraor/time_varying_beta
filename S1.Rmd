...
Likelihood Estimate (MLE) using iterated filtering. We repeat this process
**twenty** times.
...
To perform parameter inference on DGP1's fixed parameters, we employ the profile likelihood method. To illustrate the mechanics of this method, we describe each
step followed to infer the initial effective contact rate's ($\zeta$) plausible
region. We repeat this process to infer the remaining three parameters.
...
g3 <- plot_hidden_re(re_df, GBM_colour)
```
...
results_list <- list(label        = "M1 (GBM)",
                     sim_inc      = pred_inc,
...
g3 <- plot_hidden_re(re_df, CIR_colour)
```
...
results_list <- list(label        = "M2 (CIR)",
                     sim_inc      = pred_inc,
                     sim_mob      = pred_mob_effect,
                     Re_t         = re_df,
                     estimates_df = est_df)
