...
g2 + g4 + g3 + plot_layout(design = layout)

fig_path <- str_glue("./paper_plots/Fig_01_Data.pdf")

ggsave(fig_path, plot = g2 + g4 + g3 + plot_layout(design = layout),
       height = 6, width = 5)
```
...
pairs(~loglik + zeta + P_0 + tau + alpha, data = guess_result_df,
      ...
      cex = 0.5,
      labels =c("Log lik", expression(zeta), "P(0)", expression(tau),
                expression(alpha)))
```
...
g1 <- plot_wkl_fit(pred_inc, wkl_inc, y_lab = "Weekly incidence",
                   "A) Predicted incidence compared to incidence data",
                   shape = 18)
```
...
g2 <- plot_wkl_fit(pred_mob_effect, actual_mob, "Relative contact rate", 
                   "B) Relative contact rate compared to Apple's mobility index",
                   shape = 16)
```
...
```{r}
plot_filt_dist(samples_Z_df, actual_mob, "Relative contact rate") -> g2b
```
...
```{r}
plot_filt_dist_re(samples_Re_df, wkl_inc) -> g3b
```

```{r}
fig_path <- "./paper_plots/Fig_03_GBM.pdf"
...
g_demo_CIR <- plot_demo(demo_CIR, "B) Cox-Ingersoll-Ross (CIR)")

ggsave("./paper_plots/Fig_02_Simulations.pdf", 
...
g3b <- plot_filt_dist_re(samples_Re_df, wkl_inc) 
```



```{r}
ggsave("./paper_plots/Fig_04_CIR.pdf", 
