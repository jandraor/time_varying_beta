...
cutoff_list_GBM         <- profile_list_GBM
...
pairs(~loglik+zeta + P_0 + tau + alpha, data= all, pch=16)
```
...
```{r}
maxloglik                   <- max(profile_list_GBM[[var_name]]$loglik,
                                   na.rm = TRUE)

cutoff_list_GBM[[var_name]] <- maxloglik - 0.5 * qchisq(df = 1,p = 0.95)
```

```{r, fig.height = 3.5}
profile_plot(profile_list_GBM[[var_name]], var_name, 
             cutoff_list_GBM[[var_name]])
```
...
ggplot(prof_ll, aes(x = zeta, y = loglik)) +
...
plot_MCAP(prof_ll, var_name, span)
```
...
ggplot(tidy_prof, aes(x = value, y = loglik)) +
  geom_point() +
...
prof_est_df <- do.call("bind_rows", profile_list_GBM) %>% 
  filter(!is.na(loglik.se) & loglik.se < 1)

par_lims    <- summarise_pars(unk_names, cutoff_list_GBM)

prof_est_df %>% filter(loglik == max(loglik, na.rm = TRUE)) %>%
  select(-loglik, -loglik.se, -profile_over) %>% unlist() -> mle_estimate

MLE_df <- enframe(mle_estimate, name = "Parameter", value = "MLE") %>%
  filter(Parameter %in% unk_names)

par_summary <- left_join(MLE_df, par_lims, by = "Parameter")

zeta_estimates <- par_summary %>% filter(Parameter == "zeta") %>% 
  select(-Parameter) %>% unlist()

R_estimates <- estimate_r(zeta_estimates) %>% bind_rows()
R_row       <- data.frame(Parameter = "R(0)") %>% bind_cols(R_estimates)

par_summary <- bind_rows(par_summary, R_row) %>% 
  mutate(Parameter = ifelse(Parameter == "P_0", "P(0)", Parameter)) # formatting

prof_est <- par_summary %>% rename(name = Parameter,
                                   mle = MLE,
                                   ll = lower_limit,
                                   ul = upper_limit) %>% 
...
imap_dfr(mcap_list, function(vals_list, name) {
...
ggplot(all_est, aes(x = method, y = mle)) +
...
g1 <- ggplot(pred_inc, aes(x = week, y = median)) +
  geom_point(data = wkl_inc, aes(x = week, y = y)) +
...
g2 <- ggplot(pred_mob_effect, aes(x = week, y = median)) +
  geom_point(data = wkl_df, aes(x = week, y = y2)) +
  geom_line(colour = "steelblue") +
  scale_y_continuous(labels = comma) +
  geom_ribbon(alpha = 0.25, aes(ymin = lower_lim, ymax = upper_lim), 
              fill = "steelblue") +
...
pairs(~loglik + zeta + P_0 + tau + alpha + upsilon + nu, data = all, pch=16)
```
...
```{r}
maxloglik                   <- max(profile_list_CIR[[var_name]]$loglik,
                                   na.rm = TRUE)

cutoff_list_CIR[[var_name]] <- maxloglik - 0.5 * qchisq(df = 1,p = 0.95)
```

```{r, fig.height = 3.5}
profile_plot(profile_list_CIR[[var_name]], var_name,
             cutoff_list_CIR[[var_name]])
```
...
ggplot(prof_ll, aes(x = P_0, y = loglik)) +
  geom_point(alpha = 0.7, colour = "grey50") +
...
ggplot(prof_ll, aes(x = alpha, y = loglik)) +
  geom_point(alpha = 0.7, colour = "grey50") +
  geom_smooth(se = FALSE, span = span) +
  geom_hline(yintercept = cut_off_smth, colour = "red", linetype = "dotted") +
  theme_pubr()
...
prof_est_df <- do.call("bind_rows", profile_list_CIR) %>% 
  filter(!is.na(loglik.se) & loglik.se < 1)

par_lims    <- summarise_pars(unk_names, cutoff_list_CIR)
