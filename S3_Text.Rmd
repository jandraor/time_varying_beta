...
incidence counts are distributed according to the 
**Negative binomial distribution**, and that **mobility data is a proxy** 
measurement for the relative effective contact rate. We refer to this model in
S1 as **Candidate 4**. In particular, we apply _Iterated Filtering_ and the 
_Particle Filter_ to obtain, from Candidate 4, estimates (via samples) for the
effective reproductive number and other parameters that explain Ireland's first
...
ptb    <- rw_sd(P_0 = ivp(0.02), tau = 0.02, zeta = 0.02, alpha = 0.02, 
                phi = 0.02)
...
```{r}
zeta_ptb <- rw_sd(P_0 = ivp(0.02), tau = 0.02, alpha = 0.02, phi = 0.02)
...
```{r}
P_0_ptb <- rw_sd(zeta = 0.02, tau = 0.02, alpha = 0.02, phi = 0.02)
...
```{r}
alpha_ptb <- rw_sd(zeta = 0.02, tau = 0.02, P_0 = ivp(0.02), phi = 0.02)
...
```{r}
tau_ptb <- rw_sd(zeta = 0.02, alpha = 0.02, P_0 = ivp(0.02), phi = 0.02)
...
```{r}
source("./R_scripts/likelihood_funs.R")
phi_ptb <- rw_sd(zeta = 0.02, alpha = 0.02, P_0 = ivp(0.02), tau = 0.02)
