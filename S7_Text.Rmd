...
```{r}
meas_mdl   <- list("y1 ~ poisson(net_flow(C))")

est_params <- list(sd_prior("zeta", "lognormal", c(0, 1)),
                   sd_prior("upsilon", "beta", c(2, 2)),
                   sd_prior("nu", "beta", c(2, 2)),
                   sd_prior("P0", "lognormal", c(0, 1), "init"))

walk(1:n_orders, \(dly_o) {
  
  mdl_path  <- str_glue("./models/SEI3R_order_{dly_o}.stmx")
  stan_path <- str_glue("./Stan_files/SEI3R_{dly_o}_smth.stan")
  stan_file <- sd_Bayes(mdl_path, meas_mdl, est_params)
  write_file(stan_file, stan_path)
})
```
...
ad_list <- c(0.90, list(NULL), 0.95, list(NULL), list(NULL), list(NULL), 0.95,
             0.95, 0.95)

sz_list <- c(0.0001, list(NULL), 0.0001, 0.001, list(NULL), list(NULL), 0.00001, 
             0.0001, 0.0001)

mt_list <- c(11, list(NULL), 11, rep(list(NULL), 3), 12, 12, 12)

result_list <- run_stan_files(9, stan_d, folder, ad_list, sz_list, mt_list)
```

```{r, fig.height = 3.5}
plot_traces(result_list[[1]]$sf, pars)
```
...
```{r, fig.height = 3.5}
plot_traces(result_list[[2]]$sf, pars)
```
...
```{r, fig.height = 3.5}
plot_traces(result_list[[3]]$sf, pars)
```

### 4th order delay

```{r, fig.height = 3.5}
plot_traces(result_list[[4]]$sf, pars)
```

### 5th order delay

```{r, fig.height = 3.5}
plot_traces(result_list[[5]]$sf, pars)
```

### 6th order delay

```{r, fig.height = 3.5}
plot_traces(result_list[[6]]$sf, pars)
```

### 7th order delay

```{r, fig.height = 3.5}
plot_traces(result_list[[7]]$sf, pars)
```

### 8th order delay

```{r, fig.height = 3.5}
plot_traces(result_list[[8]]$sf, pars)
```

### 9th order delay

```{r, fig.height = 3.5}
plot_traces(result_list[[9]]$sf, pars)
```
...
sim_incidences_list <- imap(result_list, \(obj, i){
  construct_incidence_df(obj$sf, i)
})

Z_list <- imap(result_list, \(obj, i) {
  
  mdl    <- read_xmile(str_glue("./models/SEI3R_order_{i}.stmx"))
  stocks <- sd_stocks(mdl)|> pull(name)
  extract_timeseries_stock("Z", obj$sf, stocks, "x")
})

ll_list <- lapply(result_list, \(obj) obj$sf |> select(log_lik))

posterior_list <- purrr::map(result_list, "sf")
...
kable_df <- kable_df[, c("order", "R_0", "zeta", "nu", "upsilon", "P0")]
...
```{r}
time_list <- purrr::map(result_list, \(obj) obj$time[[1]])

imap_dfr(time_list, function(time_obj, order) {
  
  time_val <- (time_obj$toc - time_obj$tic) / 60
  print(time_val)
...
meas_mdl <- list("y1 ~ neg_binomial_2(net_flow(C), phi)")
...
mcmc_trace_highlight(demo_sf , pars = c("zeta", "phi", "nu", "upsilon"), 
                     highlight = 8, facet_args = list(labeller = label_parsed)) +
...
chains, samples from Chain 8 do not fit the data.
...
sim_inc_df   <- construct_incidence_df(posterior_df, 1) |> 
  mutate(fit = ifelse(iter >= 14001 & iter <= 16000, "no", "yes"))
```
...
```{r}
vars <- c("zeta", "nu", "upsilon", "phi", "P0", "log_lik")
...
summary_var2 |> 
  filter(converges == "yes", var %in% c("R_0", "zeta", "nu", "upsilon","P0", "phi")) |> 
  group_by(order, var) |> 
...
kable_df <- kable_df[, c("order", "R_0", "zeta", "nu", "upsilon", "P0", "phi")]
...
```{r}
purrr::map(map_list, function(sf) {
...
partial_Z_list <- imap(partial_posteriors, function(posterior_df, dly_o) {
  ...
  mdl       <- read_xmile(mdl_path)
  stocks    <- sd_stocks(mdl)
  extract_timeseries_stock("Z", posterior_df, stocks, "x")
})
```
...
dly_o     <- 1 # delay order
mdl_path  <- str_glue("./models/SEI3R_order_{dly_o}.stmx")
mdl       <- read_xmile(mdl_path, stock_list = list(S = 4937796 - 1,
                                                    P = 1))
stocks    <- sd_stocks(mdl)
...
meas_mdl <- list(str_glue("y1 ~ neg_binomial_2(net_flow(C), {phi_val})"))

est_params <- list(sd_prior("zeta", "lognormal", c(0, 1)))

stan_file <- sd_Bayes(mdl_path, 
                      meas_mdl = meas_mdl, 
                      estimated_params = est_params,
                      const_list = list(nu          = nu_val,
                                        upsilon = upsilon_val))
write_file(stan_file, stan_path)
```

```{r fit_SMTH_1_neg_binom_zeta}
fn <- file.path(folder, "sf_1.rds")
...
if(!file.exists(fn)) {
  ...
  diag_path <- file.path(folder , str_glue("diag_{dly_o}.txt"))
  diagnosis <- fit$cmdstan_diagnose()
  writeLines(diagnosis$stdout, diag_path)
  
  posterior_df <- as_draws_df(fit$draws())
  
  saveRDS(posterior_df, fn)
  
} else {
  
  posterior_df <- readRDS(fn)
}

zeta_sf   <- posterior::as_draws_array(posterior_df)
```
...
unknowns yield well-behaved chains, irrespective of particular starting points
