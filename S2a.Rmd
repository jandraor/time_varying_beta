...
Brownian Motion. Moreover, the measurement model assumes that daily incidence
counts are distributed according to the Negative binomial distribution. We refer
to this model in S1 as **Candidate 2**. In particular, we apply
_Iterated Filtering_ and the _Particle Filter_ to obtain from Candidate 2 
estimates (via samples) for the effective reproductive number and other
parameters that explain Ireland's first wave of COVID-19 in 2020.
...
ptb    <- rw.sd(P_0 = ivp(0.02), zeta = 0.02, alpha = 0.02, phi = 0.02)

fn     <- file.path(folder, "local_search.rds")

ls_obj <- local_search(pomp_mdl, params, ptb, fn, 540646174, detectCores())
```
...
pairs(~loglik+zeta + P_0 + alpha + phi,
...
runif_design(
  lower = c(zeta = 0.1, P_0 = 1,  alpha = 0.01, phi = 0.01),
  upper = c(zeta = 3,   P_0 = 30, alpha = 0.3,  phi = 1),
  nseq  = 300
) -> guesses
...
ll_obj      <- mif_ll(mifs_global, Np = 5e4, 146417047, n_cores = detectCores(),
                      fn)
```
...
pairs(~loglik + zeta + P_0 + alpha + phi, data = guess_result_df,
...
profile_design(
  ...
  lower = box[1, c("P_0", "alpha", "phi")],
  upper = box[2, c("P_0", "alpha", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
zeta_ptb <- rw.sd(P_0 = ivp(0.02), phi = 0.02, alpha = 0.02)
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 23485074, n_cores = detectCores())
```
...
profile_design(
  P_0  = seq(0.1, 15, length = 50),
  lower = box[1, c("zeta", "alpha", "phi")],
  upper = box[2, c("zeta", "alpha", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
source("./R_scripts/likelihood_funs.R")
P_0_ptb <- rw.sd(zeta = 0.02, phi = 0.02, alpha = 0.02)
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 397470024, n_cores = detectCores())
```
...
profile_results <- mif_ll(profile_mif_results, seed = 990258595, 
                          n_cores = detectCores(), filename = fn, Np = 5e4)
```
...
profile_design(
  ...
  lower  = box[1, c("zeta", "P_0", "phi")],
  upper = box[2, c("zeta", "P_0", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
alpha_ptb <- rw.sd(zeta = 0.02, phi = 0.02, P_0 = ivp(0.02))
...
profile_design(
  phi  = seq(0.01, 0.5,length = 50),
  lower  = box[1, c("zeta", "P_0", "alpha")],
  upper = box[2, c("zeta", "P_0", "alpha")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
phi_ptb <- rw.sd(zeta = 0.02, alpha = 0.02, P_0 = ivp(0.02))
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 493780858, n_cores = detectCores())
```
...
```{r}
fn              <- file.path(folder, "ifp_phi_ll.rds")
profile_results <- mif_ll(profile_mif_results, seed = 520182747, 
                          n_cores = detectCores(), filename = fn, Np = 5e4)
```
...
results_obj <- hindcast(params, pomp_mdl, fixed_params, folder, 521064455,
                        detectCores())
```
