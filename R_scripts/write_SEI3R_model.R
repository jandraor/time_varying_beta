...
# negative binomial observational model
...
write_SEI3R_model3 <- function(mdl_path, stan_path, phi_val, upsilon_val,
                               nu_val) {
  ...
  stan_fun  <- stan_ode_function(mdl_path, ODE_fn, 
                                 pars       = c("zeta"),
                                 const_list = list(upsilon = upsilon_val,
                                                   inv_nu  = 1 / nu_val))
  ...
  manual_values <- list(list(name = "S", value = "4937794 - 1"),
                        list(name = "P", value = "1"))
...
write_SEI3R_model4 <- function(mdl_path, stan_path, zeta_val, nu_val, upsilon_val) {
  ...
  stan_tp <- paste(
    ...
    "  phi_inv = 1 / phi;",
    inits,
    str_glue("  params[1] = {zeta_val};"),
    str_glue("  params[2] = {1/nu_val};"),
    str_glue("  params[3] = {upsilon_val};"),
    fun_exe_line,
    str_glue("  y1_hat[1] =  o[1, {k}]  - y0[{k}];"),
    "  for (i in 1:n_obs-1) {",
    str_glue("    y1_hat[i + 1] = o[i + 1, {k}] - o[i, {k}] + 1e-4;"),
