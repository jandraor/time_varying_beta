...
**Poisson distribution**, and that **mobility data is a proxy** measurement for
the relative effective contact rate. We refer to this model in S1 as 
**Candidate 6**. In particular, we apply _Iterated Filtering_ and the 
_Particle Filter_ to obtain, from Candidate 6, estimates (via samples) for the 
...
pairs(~loglik + zeta + P_0 + tau + alpha,
...
```{r}
source("./R_scripts/likelihood_funs.R")
zeta_ptb <- rw_sd(P_0 = ivp(0.02), tau = 0.02, alpha = 0.02)
...
```{r}
source("./R_scripts/likelihood_funs.R")
P_0_ptb <- rw_sd(zeta = 0.02, tau = 0.02, alpha = 0.02)
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 373492667, n_cores = detectCores())
```
...
```{r}
alpha_ptb <- rw_sd(zeta = 0.02, tau = 0.02, P_0 = ivp(0.02))
...
```{r}
source("./R_scripts/likelihood_funs.R")
tau_ptb <- rw_sd(zeta = 0.02, alpha = 0.02, P_0 = ivp(0.02))
...
results_obj <- hindcast(params, pomp_mdl, fixed_params, folder, 979301784,
                        detectCores())
```
...
seeds <- c(505580193, 891441972, 870160726)

lapply(1:3, function(var_pos) {
  sample_filt_dist(summary_vars[[var_pos]], folder, seeds[[var_pos]])
}) -> sample_list
```
...
plot_wkl_fit(pred_df, actual_df, y_lab = "Z[t]", "B) Relative transmission rate", 
             shape = 16, GBM_colour) -> g2

plot_filt_dist(sample_list[[2]], actual_df, "Z[t]", GBM_colour) -> g2b
```

```{r}
pred_df <- mutate(summary_list[[3]], week = 1:11)
plot_hidden_re(pred_df, GBM_colour, "C) Effective reproductive number") -> g3
plot_filt_dist_re(sample_list[[3]], actual_inc_df, GBM_colour) -> g3b
```


```{r, fig.height = 7, dev = 'cairo_pdf'}
fig_path <- "./paper_plots/Fig_04_GBM_thesis.pdf"

ggsave(fig_path, 
       plot = (g1 | g1b) / (g2 | g2b) / (g3 | g3b), dpi = "print",
       height = 6.5, width = 5, device = cairo_pdf)

ggsave("./paper_plots/Fig_04_GBM_thesis.eps", 
       plot = (g1 | g1b) / (g2 | g2b) / (g3 | g3b),
       height = 6.5, width = 5, device = cairo_ps)
