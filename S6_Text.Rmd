...
ptb    <- rw_sd(P_0 = ivp(0.02), tau = 0.02, nu = 0.02, upsilon = 0.02, 
...
```{r}
ptb <- rw_sd(P_0 = ivp(0.02), tau = 0.02, alpha = 0.02, nu = 0.02,
             upsilon = 0.02)
...
```{r}
source("./R_scripts/likelihood_funs.R")
P_0_ptb <- rw_sd(zeta = 0.02, tau = 0.02, alpha = 0.02, nu = 0.02,
                 upsilon = 0.02)
...
```{r}
ptb <- rw_sd(P_0 = ivp(0.02), tau = 0.02, zeta = 0.02, nu = 0.02,
             upsilon = 0.02)
...
```{r}
ptb <- rw_sd(P_0 = ivp(0.02), alpha = 0.02, zeta = 0.02, nu = 0.02,
             upsilon = 0.02)
...
```{r}
ptb <- rw_sd(P_0 = ivp(0.02), alpha = 0.02, zeta = 0.02, tau = 0.02,
             upsilon = 0.02)
...
write_csv(params, file.path(folder, "params.csv"))
...
fig_path <- "./paper_plots/Fig_05_CIR_thesis.pdf"

ggsave(fig_path, 
       plot = (g1 | g1b) / (g2 | g2b) / (g3 | g3b), dpi = "print",
       height = 6.5, width = 5, device = cairo_pdf)

ggsave("./paper_plots/Fig_05_CIR_thesis.eps", 
