...
Brownian Motion. Moreover, this candidate's measurement model assumes that daily
incidence counts are distributed according to the Negative binomial 
distribution, and that mobility data is a proxy measurement for the relative 
effective contact rate. We refer to this model in S1 as **Candidate 4**. In 
particular, we apply _Iterated Filtering_ and the _Particle Filter_ to obtain 
from Candidate 4 estimates (via samples) for the effective reproductive number 
...
labels_GBM     <- c("Log lik", expression(zeta), expression("P"[0]),
                 expression(tau), expression(alpha), expression(phi))  
```
...
ls_obj <- local_search(pomp_mdl, params, ptb, fn, 126842004, detectCores())
```
...
pairs(~loglik + zeta + P_0 + tau + alpha + phi,
...
(in contrast with the 20 runs in Section 2.1). We refer to this step as
...
runif_design(
  lower = c(zeta = 0.1, P_0 = 1,  tau = 0.05, alpha = 0.01, phi = 0.1),
  upper = c(zeta = 3,   P_0 = 30, tau = 3, alpha = 0.3, phi = 5),
  nseq  = 300
) -> guesses
...
ll_obj      <- mif_ll(mifs_global, Np = 5e4, 983707478, 
...
pairs(~loglik + zeta + P_0 + tau + alpha + phi, data = guess_result_df,
...
profile_design(
  ...
  lower = box[1, c("P_0" , "tau", "alpha", "phi")],
  upper = box[2, c("P_0", "tau", "alpha", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
zeta_ptb <- rw.sd(P_0 = ivp(0.02), tau = 0.02, alpha = 0.02, phi = 0.02)
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 762115017, n_cores = detectCores())
```
...
profile_results <- mif_ll(profile_mif_results, seed = 744170621,
                          n_cores = detectCores(), filename = fn, Np = 5e4)
```
...
the Profile Likelihood method, and its refined version, the Monte 
Carlo-adjusted profile. The red dashed line indicates the cut-off at the 95% 
confidence level.
...
profile_design(
  P_0  = seq(0.1, 30, length = 50),
  lower = box[1, c("zeta" , "tau", "alpha", "phi")],
  upper = box[2, c("zeta", "tau", "alpha", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
P_0_ptb <- rw.sd(zeta = 0.02, tau = 0.02, alpha = 0.02, phi = 0.02)
...
profile_design(
  ...
  lower  = box[1, c("zeta" , "tau", "P_0", "phi")],
  upper = box[2, c("zeta", "tau", "P_0", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
alpha_ptb <- rw.sd(zeta = 0.02, tau = 0.02, P_0 = ivp(0.02), phi = 0.02)
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 784920424, n_cores = detectCores())
```
...
profile_results <- mif_ll(profile_mif_results, seed = 872114710, 
                          n_cores = detectCores(), filename = fn, Np = 5e4)
```
...
profile_design(
  tau   = seq(0, 0.15, length = 50),
  lower = box[1, c("zeta" , "alpha", "P_0", "phi")],
  upper = box[2, c("zeta", "alpha", "P_0", "phi")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
tau_ptb <- rw.sd(zeta = 0.02, alpha = 0.02, P_0 = ivp(0.02), phi = 0.02)
...
ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             ...
                             filename = fn_ifp,
                             seed = 118253804, n_cores = detectCores())
```
...
```{r GBM_phi_raw_ll, fig.height = 3.5, fig.align ='center'}
raw_likelihood(guess_result_df, var_name, GBM_colour)
```
...
profile_design(
  phi   = seq(0, 0.9,length = 50),
  lower = box[1, c("zeta" , "alpha", "P_0", "tau")],
  upper = box[2, c("zeta", "alpha", "P_0", "tau")],
  nprof = 20, type = "sobol"
) -> guesses
```
...
```{r}
source("./R_scripts/likelihood_funs.R")
phi_ptb <- rw.sd(zeta = 0.02, alpha = 0.02, P_0 = ivp(0.02), tau = 0.02)

fn_ifp     <- file.path(folder, "ifp_phi.rds" )

ifp_obj <- iter_filt_profile(mf1 = mf1, 
                             guesses = guesses,
                             fixed_params = fixed_params,
                             perturbations = phi_ptb,
                             filename = fn_ifp,
                             seed = 46042964, n_cores = detectCores())
```
...
```{r plot_GBM_traces_phi, message = FALSE, fig.height = 3.5}
profile_mif_results <- extract_mif_results(ifp_obj)
...
results_obj <- hindcast(params, pomp_mdl, fixed_params, folder, 521064457,
                        detectCores())
```
...
```{r}
plot_daily_fit(summary_list[[1]], rename(obs_df, y = y1), "C[t]^d",
               "Incidence", 18, GBM_colour) -> g1
```

```{r}
plot_daily_fit(summary_list[[2]], rename(obs_df, y = y2), "Z[t]",
               "Relative transmission rate", 16, GBM_colour) -> g2
```

```{r}
plot_daily_re(summary_list[[3]], GBM_colour, 
              "Effective reproduction number") -> g3
```
