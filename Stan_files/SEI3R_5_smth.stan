functions {
  vector SEI3R(real time, vector y, real[] params) {
    ...
    real P_to_C;
    real S_to_E;
    E_to_P = 0.3333333333*y[7];
    P_to_A = (1-0.73)*0.4761904762*y[8];
    P_to_I = 0.73*0.4761904762*y[8];
    A_to_R = 0.2*y[11];
    I_to_R = 0.3448275862*y[9];
    lambda = (params[1]*y[1]/4937796)*(y[9]+y[8]+0.5*y[11]);
    adjust_Z = (y[2]-y[1])/(params[2]/5);
    adjust_Z_2 = (y[3]-y[2])/(params[2]/5);
    adjust_Z_3 = (y[4]-y[3])/(params[2]/5);
    adjust_Z_4 = (y[5]-y[4])/(params[2]/5);
    adjust_Z_5 = (params[3]-y[5])/(params[2]/5);
    P_to_C = P_to_I;
    S_to_E = lambda*y[6];
    dydt[1] = adjust_Z;
    dydt[2] = adjust_Z_2;
    ...
    dydt[5] = adjust_Z_5;
    dydt[6] = -S_to_E;
    dydt[7] = S_to_E-E_to_P;
    dydt[8] = E_to_P-P_to_A-P_to_I;
...
transformed parameters{
  ...
  y1_hat[1] =  o[1, 12]  - y0[12];
  for (i in 1:n_obs-1) {
    y1_hat[i + 1] = o[i + 1, 12] - o[i, 12] + 1e-4;
  }
}
model {
  zeta    ~ lognormal(0, 1);
